...
const RALPHY_LOG = '/home/sisi/projects/spermrace/ralphy.log'; // Added ralphy log support
...
function watchRalphLog() {
  ...
  if (!existsSync(targetLog)) return;
  
  // Initial read
  try {
    const content = readFileSync(targetLog, 'utf8');
    logOffset = content.length;
  } catch {} // Ignore errors during initial read

  const watcher = watch(targetLog, { persistent: true });
  watcher.on('change', path => {
    try {
      const stats = execSync(`stat -c%s "${targetLog}"`).toString().trim();
      const size = parseInt(stats);
      if (size < logOffset) logOffset = 0; // Rotated
      
      const newContent = execSync(`tail -c +${logOffset + 1} "${targetLog}"`, { encoding: 'utf8' });
      logOffset = size;
      
      if (newContent.trim()) {
        broadcast({ type: 'orchestrator_log', text: newContent });
      }
    } catch {} // Ignore errors during change detection
  });
}
...
function getAgentFiles(agentDir) {
  ...
  } catch { return []; } // Ignore errors during git status check
}
...
function getAllAgents() {
  ...
  try {
    return readdirSync(WORKTREE_DIR)
      ...
      .filter(a => a.id);
  } catch { return []; } // Ignore errors during directory reading
}
...
function pollCommands() {
  setInterval(() => {
    ...
    } catch {} // Ignore errors during command polling
  }, 2000);
}
