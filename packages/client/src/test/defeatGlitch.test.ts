...
describe('Defeat Screen Glitch Aesthetic', () => {
  ...
  describe('showCriticalFailure', () => {
    ...
    it('should display CRITICAL FAILURE text', () => {
      ...
      const title = document.getElementById('critical-failure-title');
      expect(title?.textContent).toBe('CRITICAL FAILURE');
    });

    it('should apply glitch animations to the text', () => {
      gameEffects.showCriticalFailure();

      const glitchElement = document.getElementById('critical-failure-title');
      expect((glitchElement as any)?.style?.animation || '').toContain('rgb-split');
    });
  ...
  describe('Glitch Effect Properties', () => {
    ...
    it('should use Orbitron font for glitch text', () => {
      ...
      const glitchElement = document.getElementById('critical-failure-title');
      expect((glitchElement as any)?.style?.fontFamily || '').toContain('Orbitron');
    });
  });
  ...
  describe('RGB Split Effect', () => {
    it('should apply red and cyan color offset to glitch text', () => {
      ...
      const glitchElement = document.getElementById('critical-failure-title');
      const color = (glitchElement as any)?.style?.color || '';
