...
export class HudManager {
  ...
  createHUD() {
    ...
    aliveSection.className = 'alive-section';
  ...
  updateAliveCount(count: number) {
    ...
    this.aliveCountEl.classList.add('alive-counter');
    this.aliveCountEl.classList.remove('critical', 'warning');
    if (count <= 2) {
      const color = this.theme === 'tactical' ? '#ff3333' : '#ef4444';
      this.aliveCountEl.style.color = color;
      if (this.theme === 'tactical') this.aliveCountEl.classList.add('critical');
    } else if (count <= 4) {
      const color = this.theme === 'tactical' ? '#ffaa00' : '#fbbf24';
      this.aliveCountEl.style.color = color;
      if (this.theme === 'tactical') this.aliveCountEl.classList.add('warning');
    } else {
