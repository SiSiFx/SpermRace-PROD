...
class SpermRaceGame {
  ...
  createCar(x: number, y: number, color: number, type: string): Car {
    ...
    const car: Car = {
      ...
      tailWaveT: 0,
      tailLength: 40,
      tailSegments: /iPhone|iPad|iPod|Android/i.test(navigator.userAgent) ? 14 : 20,
      tailAmplitude: 2.4,
    ...
    const headSize = type === 'player' ? 8 : 9.5;
    const strokeWidth = type === 'player' ? 1.5 : 2;
  ...
  updateCarVisuals(car: Car, deltaTime: number) {
    ...
    if (car.tailGraphics) {
      const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);
      const speedNorm = Math.max(0, Math.min(1.35, car.speed / Math.max(1, car.boostSpeed)));
      const segs = isMobile ? 14 : 22;
      const baseLen = (car.isBoosting ? 54 : 40) * sizeMul;
      const baseAmp = (car.isBoosting ? 3.1 : 2.2) * sizeMul;
      const waveSpeed = 8 + speedNorm * 9 + (car.isBoosting ? 4 : 0);
      car.tailWaveT = (car.tailWaveT || 0) + deltaTime * waveSpeed;
      const turnBias = Math.max(-0.9, Math.min(0.9, normalizeAngle(((car as any).targetAngle ?? car.angle) - car.angle)));
      const tail = car.tailGraphics;
      tail.visible = true;
      tail.clear();
      ...
      for (let i = 1; i <= segs; i++) {
        ...
        const wave = Math.sin((t * 9.5) - (car.tailWaveT || 0)) * baseAmp * envelope;
        const y = wave + (turnBias * 6.5 * sizeMul * t * t);
        const w = (3.2 * (1 - t) + 0.45) * sizeMul;
        const a = 0.82 * (1 - t * 0.58);
        tail.moveTo(prevX, prevY);
        tail.lineTo(x, y);
        tail.stroke({ width: w, color: car.color, alpha: a, cap: 'round', join: 'round' });
        prevX = x;
        prevY = y;
      }

      // Soft tail glow to keep it readable against dark background.
      tail.moveTo(0, 0);
      tail.lineTo(prevX, prevY);
      tail.stroke({ width: 1.5 * sizeMul, color: 0xffffff, alpha: 0.18, cap: 'round', join: 'round' });
    }

    if (car.headGraphics) {
      const rx = 8.8 * sizeMul;
      const ry = 6.2 * sizeMul;
      const coreColor = (car.color & 0xfefefe) >> 1;
      car.headGraphics.clear();
      car.headGraphics.ellipse(0, 0, rx, ry).fill({ color: car.color, alpha: 1.0 });
      car.headGraphics.ellipse(0, 0, rx, ry).stroke({ width: 1.2 * sizeMul, color: 0xffffff, alpha: 0.2 });
      car.headGraphics.ellipse(-rx * 0.18, 0, rx * 0.36, ry * 0.34).fill({ color: coreColor, alpha: 0.36 });
      car.headGraphics.circle(rx * 0.27, -ry * 0.25, ry * 0.22).fill({ color: 0xffffff, alpha: 0.34 });
      if (car.isBoosting) {
        car.headGraphics.ellipse(0, 0, rx + 3.8, ry + 3).fill({ color: car.color, alpha: 0.18 });
      }
      if (buffActive) {
  ...
  renderTrail(trail: Trail) {
    ...
    const ptsRaw: Array<{x:number;y:number;time:number;isBoosting:boolean;expiresAt?:number}> = [];
    for (let i = 0; i < trail.points.length; i += step) {
      const p = trail.points[i];
      if (!Number.isFinite(p?.x) || !Number.isFinite(p?.y)) continue;
      if (isOnline && typeof (p as any).expiresAt === 'number') {
        if ((p as any).expiresAt > now) ptsRaw.push(p as any);
        continue;
      }
      const age = (now - p.time) / 1000;
      if (age <= 8.5) ptsRaw.push(p as any);
    }
    if (ptsRaw.length < 2) return;

    const timeKey = (p: any) => (typeof p.expiresAt === 'number' ? p.expiresAt : p.time);
    const pts = ptsRaw.slice().sort((a, b) => timeKey(a) - timeKey(b));
    ...
    const breakGap = isPlayerTrail ? 200 : 240;
    const breakGapSq = breakGap * breakGap;
    const segments: Array<Array<{x:number;y:number;time:number;isBoosting:boolean;expiresAt?:number}>> = [];
    let currentSegment: Array<{x:number;y:number;time:number;isBoosting:boolean;expiresAt?:number}> = [pts[0]];
    for (let i = 1; i < pts.length; i++) {
      const prev = currentSegment[currentSegment.length - 1];
      const p = pts[i];
      const dx = p.x - prev.x;
      const dy = p.y - prev.y;
      if ((dx * dx + dy * dy) > breakGapSq) {
    ...
    // Reach the head if the last segment is close enough; avoid teleport lines.
    const tailSeg = segments[segments.length - 1];
    const tailLast = tailSeg[tailSeg.length - 1];
    const dxHead = car.x - tailLast.x;
    const dyHead = car.y - tailLast.y;
    if ((dxHead * dxHead + dyHead * dyHead) < (135 * 135)) {
      tailSeg.push({ x: car.x, y: car.y, time: now, isBoosting: car.isBoosting });
    }

    const drawSmoothSegment = (poly: Array<{x:number;y:number}>, width: number, alpha: number) => {
      if (poly.length < 2) return;
      trail.graphics.moveTo(poly[0].x, poly[0].y);
      if (poly.length === 2) {
        trail.graphics.lineTo(poly[1].x, poly[1].y);
      } else {
        for (let i = 1; i < poly.length - 1; i++) {
          const cx = (poly[i].x + poly[i + 1].x) * 0.5;
          const cy = (poly[i].y + poly[i + 1].y) * 0.5;
          if (typeof (trail.graphics as any).quadraticCurveTo === 'function') {
            (trail.graphics as any).quadraticCurveTo(poly[i].x, poly[i].y, cx, cy);
          } else {
            trail.graphics.lineTo(poly[i].x, poly[i].y);
          }
        }
        const lastP = poly[poly.length - 1];
        trail.graphics.lineTo(lastP.x, lastP.y);
      }
      trail.graphics.stroke({ width, color: trailColor, alpha, cap: 'round', join: 'round' });
    };

    for (const seg of segments) {
      // Densify short/medium gaps so trails look fluent instead of dotted.
      const dense: Array<{x:number;y:number}> = [seg[0]];
      for (let i = 1; i < seg.length; i++) {
        ...
        const dy = b.y - a.y;
        const dist = Math.hypot(dx, dy);
        const interpSteps = Math.max(0, Math.min(5, Math.floor(dist / 18)));
        for (let s = 1; s <= interpSteps; s++) {
          const t = s / (interpSteps + 1);
          dense.push({ x: a.x + dx * t, y: a.y + dy * t });
        }
        dense.push({ x: b.x, y: b.y });
      }

      // Two-layer stroke: soft glow + core line.
      drawSmoothSegment(dense, baseWidth + 1.25, alphaStart * 0.22);
      drawSmoothSegment(dense, baseWidth, alphaStart);
    }

    // Proximity glow (subtle, thinner) using segment checks
    if (this.player && trail.car !== this.player && segments.length) {
      for (const seg of segments) {
        for (let i = 1; i < seg.length; i++) {
          const a = seg[i - 1];
          const b = seg[i];
          ...
          if (dist < 60) {
            const glowAlpha = Math.max(0, 0.24 * (1 - dist / 60));
            trail.graphics
              .moveTo(a.x, a.y)
              .lineTo(b.x, b.y)
              .stroke({ width: baseWidth + 0.9, color: this.theme.enemyGlow, alpha: glowAlpha, cap: 'round', join: 'round' });
          }
        }
      }
    }
