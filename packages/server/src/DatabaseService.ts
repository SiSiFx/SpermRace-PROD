...
interface Player {
  ...
  elo_rating: number; // skill rating for matchmaking
  created_at: string;
}
...
export class DatabaseService {
  ...
  ensurePlayer(walletAddress: string): void {
    const stmt = this.db.prepare(`
      INSERT INTO players (wallet_address, elo_rating)
      VALUES (?, 1200)
  ...
  getPlayerElo(walletAddress: string): number {
    this.ensurePlayer(walletAddress);
    const stmt = this.db.prepare(`
      SELECT elo_rating FROM players WHERE wallet_address = ?
    `);
    const result = stmt.get(walletAddress) as { elo_rating: number } | undefined;
    return result?.elo_rating ?? 1200;
  }
  ...
  getPlayersElo(walletAddresses: string[]): Map<string, number> {
    ...
    const stmt = this.db.prepare(`
      SELECT wallet_address, elo_rating FROM players
      WHERE wallet_address IN (${placeholders})
    `);
    const results = stmt.all(...walletAddresses) as Array<{ wallet_address: string; elo_rating: number }>;

    for (const result of results) {
      eloMap.set(result.wallet_address, result.elo_rating);
    }
