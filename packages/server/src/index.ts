...
const SESSION_TOKEN_BIND_IP = (process.env.SESSION_TOKEN_BIND_IP || '1').toLowerCase() !== '0';
const IS_PRODUCTION = (process.env.NODE_ENV || '').toLowerCase() === 'production';
const TRUST_PROXY = (process.env.TRUST_PROXY ?? (IS_PRODUCTION ? '1' : '0')).toLowerCase() !== '0';
const OPS_TOKEN = (process.env.OPS_TOKEN || '').trim();
const OPS_ALLOW_LOCAL = (process.env.OPS_ALLOW_LOCAL || '1').toLowerCase() !== '0';
...
const httpSiwsChallenges = new Map<string, HttpSiwsChallenge>();
...
function isOpsAuthorized(req: Request): boolean {
  try {
    const ip = getClientIp(req as any);
    if (OPS_ALLOW_LOCAL && ip && isLocalIp(ip)) return true;
    if (!OPS_TOKEN) return !IS_PRODUCTION;

    const headerToken = String(req.header('x-ops-token') || '').trim();
    if (headerToken && headerToken === OPS_TOKEN) return true;

    const auth = String(req.header('authorization') || '').trim();
    if (auth.toLowerCase().startsWith('bearer ')) {
      const bearerToken = auth.slice('bearer '.length).trim();
      if (bearerToken && bearerToken === OPS_TOKEN) return true;
    }
  } catch { }
  return false;
}

function cleanupHttpSiwsChallenges(now = Date.now()): void {
  try {
    for (const [nonce, challenge] of httpSiwsChallenges.entries()) {
      if (!challenge || challenge.consumed || now > challenge.expiresAt) {
...
function cleanupSessionTokens(now = Date.now()): void {
  try {
    for (const [token, data] of sessionTokens.entries()) {
      if (!data || now > data.expiresAt) sessionTokens.delete(token);
    }
  } catch { }
}
...
app.get('/api/readyz', async (_req, res) => {
  ...
  try {
    ...
    const configured = !!prize;
    if (isOpsAuthorized(_req as any)) {
      res.json({ ok: true, rpc: { ok: !!blockhash, endpoint: rpcEndpoint }, prizePool: { address: prize || null, balance: prizeBalance, configured } });
      return;
    }
    // Public-safe readiness: avoid leaking internal endpoints or wallet addresses.
    res.json({ ok: true, rpc: { ok: !!blockhash }, prizePool: { configured } });
  } catch (e: any) {
...
app.post('/api/siws-auth', limiterSensitive, async (req, res) => {
  try {
    ...
    if (!publicKey || !signedMessage || !message || !nonce) {
      res.status(400).json({ ok: false, error: 'Missing required fields' });
      return;
    }

    const now = Date.now();
    cleanupHttpSiwsChallenges(now);
    cleanupSessionTokens(now);

    const challenge = httpSiwsChallenges.get(nonce);
    if (!challenge || challenge.consumed || now > challenge.expiresAt) {
    ...
    if (challenge.clientIp && SESSION_TOKEN_BIND_IP) {
      const requestIp = getClientIp(req);
      if (requestIp && requestIp !== challenge.clientIp) {
        res.status(401).json({ ok: false, error: 'SIWS challenge origin mismatch' });
        return;
      }
    }

    // Verify signature + bind to expected nonce from issued challenge
    const isValid = AuthService.verifySignature(publicKey, signedMessage, message, { expectedNonce: nonce });
...
app.get('/api/siws-challenge', limiterSensitive, (req, res) => {
  try {
    ...
    const expiresAt = now + SIWS_CHALLENGE_TTL_MS;
    httpSiwsChallenges.set(nonce, {
      nonce,
      message,
      createdAt: now,
      expiresAt,
    ...
    res.json({ ok: true, message, nonce, expiresIn: SIWS_CHALLENGE_TTL_MS });
  } catch (e: any) {
    res.status(500).json({ ok: false, error: e?.message || String(e) });
  }
});
...
wss.on('connection', (ws: WebSocket, req: any) => {
  ...
  // Check for session token in WS handshake (HTTP auth flow)
  ...
  try {
    ...
    const tokenProtocol = protocols.find((entry: string) => entry.startsWith('token.'));
    const tokenFromProtocol = tokenProtocol ? tokenProtocol.slice('token.'.length) : null;
    sessionToken = tokenFromProtocol || tokenFromQuery;
    ...
    if (sessionToken) {
      ...
      const requestIp = getClientIp(req);
      const ipMismatch = SESSION_TOKEN_BIND_IP
        && !!sessionData?.clientIp
        && !!requestIp
        && sessionData.clientIp !== requestIp;
      if (sessionData && !ipMismatch && now <= sessionData.expiresAt) {
        authenticatedPlayerId = sessionData.playerId;
        // Consume immediately: token is one-time handoff to reduce replay risk.
        sessionTokens.delete(sessionToken);
        log.info(`[WS] Client authenticated via HTTP token: ${maskPk(authenticatedPlayerId)}`);
      } else {
        if (ipMismatch) {
          log.warn(`[WS] Session token rejected due to client IP mismatch: ${sessionToken.slice(0, 8)}…`);
        }
...
async function handleClientMessage(message: any, ws: WebSocket): Promise<void> {
  ...
  switch ((message as any).type) {
    ...
    case 'authenticate': {
      ...
      const ok = !expired
        && expected === nonce
        && AuthService.verifySignature(publicKey, signedMessage, original, { expectedNonce: expected });
      if (!ok) {
