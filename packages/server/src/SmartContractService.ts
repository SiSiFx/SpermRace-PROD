...
export class SmartContractService {
  ...
  private prizePoolKeypair: Keypair | null;
  ...
  constructor() {
    ...
    if (secret) {
      try {
        ...
        if (!secretBytes) throw new Error('Invalid PRIZE_POOL_SECRET_KEY format');
  ...
  getPrizePoolAddressBase58(): string {
  ...
  async getPrizePoolBalanceLamports(): Promise<number> {
    try {
      const pubkey = this.prizePoolKeypair?.publicKey || PRIZE_POOL_WALLET;
      return await this.connection.getBalance(pubkey, { commitment: 'processed' as any });
    } catch {
      return -1;
    }
  }
   ...
   */
  async getEntryFeeInSol(entryFeeUsd: EntryFeeTier): Promise<number> {
    const solPriceUsd = await this.fetchSolPriceUsd();
    if (!solPriceUsd || solPriceUsd <= 0) throw new Error('SOL price fetch failed');
  ...
  private async fetchSolPriceUsd(): Promise<number> {
    ...
    const sources: Array<{ url: string; pick: (j: any) => number | null }> = [
  ...
  async getEntryFeeInLamports(entryFeeUsd: EntryFeeTier): Promise<number> {
   ...
   */
  async createEntryFeeTransactionBase64(playerPublicKey: PublicKey, lamports: number): Promise<{ txBase64: string; lamports: number; recentBlockhash: string; prizePool: string; }>{
   ...
   */
  async verifyEntryFee(signature: string, expectedLamports: number): Promise<{ ok: boolean; payer?: string; error?: string }> {
    ...
    try {
      const parsed = await this.connection.getParsedTransaction(
        signature,
        { maxSupportedTransactionVersion: 0, commitment: 'confirmed' } as any
      );
      ...
      if (parsed && parsed.meta && !parsed.meta.err) {
        const instructions: any[] = (parsed.transaction as any).message?.instructions || [];
        for (const ix of instructions) {
          if (ix?.program === 'system' && ix?.parsed?.type === 'transfer') {
            ...
            if (destination === PRIZE_POOL_WALLET.toBase58() && lamports === expectedLamports) {
              const keys: any[] = (parsed.transaction as any).message?.accountKeys || [];
    ...
    try {
      ...
      try {
        const raw = (message.accountKeys as any);
        if (Array.isArray(raw)) {
          accountKeys = raw.map((k: any) => (typeof k === 'string' ? k : k.pubkey?.toBase58?.() || String(k)));
        } else if (raw?.staticAccountKeys) {
          accountKeys = raw.staticAccountKeys.map((k: any) => (typeof k === 'string' ? k : k.toBase58?.() || String(k)));
        }
      } catch {}
      const prizeIndex = accountKeys.findIndex((k) => k === PRIZE_POOL_WALLET.toBase58());
      if (prizeIndex === -1) return { ok: false };
      const pre = tx.meta.preBalances;
      const post = tx.meta.postBalances;
      const prizeDelta = (post[prizeIndex] ?? 0) - (pre[prizeIndex] ?? 0);
      if (prizeDelta !== expectedLamports) return { ok: false };
      const payer = accountKeys[0];
      const payerIndex = 0;
      const payerDelta = (pre[payerIndex] ?? 0) - (post[payerIndex] ?? 0);
      if (payerDelta < expectedLamports) return { ok: false };
   ...
   */
  async refundPlayer(playerPublicKey: string, lamports: number): Promise<string> {
  ...
  async payoutPrizeLamports(winnerPublicKey: PublicKey, lamports: number, platformFeeBps = 1500): Promise<string> {
    if (!this.prizePoolKeypair) throw new Error('Prize pool key not configured');
    // Preflight balance check
    const prizeBalance = await this.connection.getBalance(this.prizePoolKeypair.publicKey, { commitment: 'processed' as any });
