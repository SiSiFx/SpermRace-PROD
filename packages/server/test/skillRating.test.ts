...
describe('Skill Rating System', () => {
  ...
  describe('ELO Calculation', () => {
    ...
    it('should calculate expected score correctly for higher rated player', () => {
      ...
      expect(expectedScore).toBeGreaterThanOrEqual(0.5);
    ...
    it('should calculate expected score correctly for lower rated player', () => {
      ...
      expect(expectedScore).toBeGreaterThanOrEqual(0.0);
      expect(expectedScore).toBeLessThanOrEqual(0.5);
    });
    ...
    it('should return expected score close to 1 for much higher rated player', () => {
      ...
      expect(expectedScore).toBeGreaterThanOrEqual(0.9);
    });
  });
  ...
  describe('Skill Rating Change Calculation', () => {
    it('should increase winner rating and decrease loser rating', () => {
      ...
      expect(winnerNewRating).toBeGreaterThanOrEqual(1200);
      expect(loserNewRating).toBeLessThanOrEqual(1200);
    });
    ...
    it('should give smaller rating change when favorite wins', () => {
      ...
      expect(winnerChange).toBeLessThanOrEqual(loserChange);
    });
    ...
    it('should give larger rating change when underdog wins', () => {
      ...
      expect(winnerChange).toBeGreaterThanOrEqual(loserChange);
    });
    ...
    it('should adjust ratings faster for new players', () => {
      ...
      expect(newPlayerChange).toBeGreaterThanOrEqual(veteranChange);
    });
  });
  ...
  describe('Database Integration', () => {
    ...
    it('should update skill ratings after a game', () => {
      ...
      expect(winnerAfter.skill_rating).toBeGreaterThanOrEqual(winnerBefore.skill_rating);

      // Loser's rating should decrease
      expect(loser1After.skill_rating).toBeLessThanOrEqual(loser1Before.skill_rating);
    });
    ...
    it('should return skill rating leaderboard', () => {
      ...
      const updateRating = (wallet: string, rating: number) => {
        (db as any).db.prepare('UPDATE players SET skill_rating = ?, total_games = 1 WHERE wallet_address = ?').run(rating, wallet);
      };
      ...
      // Refresh leaderboard cache to include newly updated ratings
      ;(db as any).refreshCache('skillRating', 10);
  ...
  describe('Rating Consistency', () => {
    ...
    it('should handle multiple sequential games correctly', () => {
      ...
      expect(p1Stats.skill_rating).toBeGreaterThanOrEqual(initialRating);

      // p2 should have lost rating
      expect(p2Stats.skill_rating).toBeLessThanOrEqual(initialRating);

      // Rating gap should be positive
      expect(p1Stats.skill_rating - p2Stats.skill_rating).toBeGreaterThanOrEqual(0);
    });
  });
});
