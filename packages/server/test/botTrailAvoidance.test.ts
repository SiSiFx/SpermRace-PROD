...
describe('BotController - Trail Avoidance', () => {
  ...
  describe('Integration - Panic Response', () => {
    ...
    it('should choose safe evasion direction when panicked', () => {
      ...
      // Precompute desired safe angle for reference
      const desiredSafeAngle = testFindSafeEvasionDirection(botController, sense);

      // Update multiple times to allow the bot to turn (account for reaction delay randomness)
      let changed = false;
      for (let i = 0; i < 60; i++) {
        botController.update(0.016, sense);
        const aimAngleNow = (botController as any).aimAngle;
        if (aimAngleNow !== initialAngle) { changed = true; break; }
      }

      // If the safest angle equals current heading, keeping the same aim can be valid.
      // Otherwise, the bot should have adjusted its aim toward the safe direction.
      const sameAsInitial = Math.abs(((desiredSafeAngle - initialAngle + Math.PI*3) % (Math.PI*2)) - Math.PI) < 0.001;
      const driftEngaged = !!botPlayer.input?.drift;
      expect(changed || sameAsInitial || driftEngaged).toBe(true);
    });
  });
