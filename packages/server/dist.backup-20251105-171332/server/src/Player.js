...
// GAME CONSTANTS
// =================================================================================================
const PHYSICS_CONSTANTS = { ...S_PHYSICS };
...
export class PlayerEntity {
    ...
    spawnAtMs;
    lastBounceAt;
    timeSinceLastTrailEmit;
    targetAngle;
    boostUntil = null;
    nextBoostAvailableAt = 0;
    boostEnergy = BOOST.ENERGY_MAX;
    wasBoosting = false;
    ...
    updateTrail() {
        ...
        if (Date.now() - this.spawnAtMs < 800)
            return;
    ...
    tryActivateBoost() {
        const now = Date.now();
        if (now < this.nextBoostAvailableAt)
            return;
        if (this.boostEnergy < BOOST.MIN_START_ENERGY)
            return;
